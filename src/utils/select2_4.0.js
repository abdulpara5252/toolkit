  ...
  var S2 = (function () {
    ...
    S2.define(
      ...
      function ($, BaseSelection, Utils) {
        ...
        MultipleSelection.prototype.update = function (data) {
          this.clear();
          //Added For Multi Selection Disabled option not consider in total count
          var data = $.grep(data, function (v) {
          ...
          if (data.length > 0) {
            //Added For Multi Selection Disabled option not consider in total count
            var totallength = 0;
            if (this.options.options.isCountOnlyChild == 1) {
              this.$element
                .find("option[data-is-parent=false]")
                .each(function (index) {
                  var selectElement = $(this);
                  if (!selectElement.is(":disabled")) totallength++;
                });

              data = jQuery.grep(data, function (n) {
                return $(n.element).data("is-parent") == false;
              });
            } else {
            ...
            //var $selection = data.length + " Selected";
            // var totallength = this.$element.find("option").length;
            if (totallength == data.length) {
              var formatted = "<span>All Selected</span>";
            } else if (data.length == 1) {
              var formatted =
                "<span>" + (data[0].title || data[0].text) + "</span>";
            } else {
              var formatted = "<span>" + data.length + " Selected</span>";
            }
            $rendered.empty().append(formatted);
    ...
    S2.define("select2/selection/placeholder", ["../utils"], function (Utils) {
      function Placeholder(decorated, $element, options) {
        ...
        searchInputPlaceholder = options.get("searchInputPlaceholder");
    ...
    S2.define(
      ...
      function ($, Utils, KEYS) {
        ...
        Search.prototype.bind = function (decorated, container, $container) {
          ...
          decorated.call(this, container, $container);
          if (self.options.options.selectAll != 1) {
            container.on("open", function () {
            ...
            container.on("close", function () {
              ...
              self.$search.val("");
              self.$search.focus();
              $(".vg_clear_search").removeClass("show");
            });
        ...
        Search.prototype.handleSearch = function () {
          if (this.options.options.selectAll != 1) {
            this.resizeSearch();
    ...
    S2.define(
      ...
      function ($, Utils) {
        ...
        selectAll.prototype.render = function (decorated) {
          ...
          var $selectall = $(
            '<span class="select2-search select2-selectAll--dropdown">' +
          ...
          this.$selectall = $selectall.find("a.selectall");
          this.$selectnone = $selectall.find("a.selectnone");
        ...
        selectAll.prototype.bind = function (decorated, container, $container) {
          ...
          this.$selectall.on("click", function (evt) {
            var _this = this;
            var data = [];
            self.handleselectall(evt);
          });
          this.$selectnone.on("click", function (evt) {
            self.handleselectnone(evt);
          });
        };
        selectAll.prototype.handleselectall = function (evt) {
          this.$dropdown
            .find(".select2-results__option[aria-selected='false']")
            .attr("aria-selected", "true");
          this.$element.find("option").attr("selected", "selected");

          var newVal = null;
          newVal = $.map(this.$element.find("option"), function (obj) {
            return $(obj).val().toString();
          });
          this.$element.val(newVal);
          this.$element.trigger("change");
        };
        selectAll.prototype.handleselectnone = function (evt) {
          this.$dropdown
            .find(".select2-results__option[aria-selected='true']")
    ...
    S2.define(
      ...
      function ($, Utils) {
        ...
        Search.prototype.render = function (decorated) {
          ...
          var $search = $(
            '<span class="select2-search select2-search--dropdown">' +
              ...
              ' spellcheck="false" role="textbox" /><span class="vg_clear_search"></span>' +
              "</span>"
          );
          ...
          this.$search.attr("placeholder", searchInputPlaceholder);
        ...
        Search.prototype.bind = function (decorated, container, $container) {
          ...
          //if (false == /iPad/i.test(navigator.userAgent)) {
          container.on("open", function () {
            self.$search.attr("tabindex", 0);
        ...
        Search.prototype.handleSearch = function (evt) {
          if (!this._keyUpPrevented) {
            ...
            $(".vg_clear_search").on("click", function () {
              self.$search.val("");
              self.trigger("query", {
                term: "",
              });
